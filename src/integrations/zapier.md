...
|---------|-------|----------|
| **New Review** | [`review.created`](../events/review-created.md) | A new App Store / Google Play review (filter by store, app, storefront, star band). |
| **Rating Dropped** | [`rating.dropped`](../events/rating-dropped.md) | The app's aggregate rating crosses a threshold or falls by a delta. |
...
3. **Configure the trigger** — choose the store, app, storefront, and filters (star band for
   reviews; threshold/delta for ratings). These map directly to the
   [filter options](../api/filters.md).
...
5. **Add an action** — Slack, Sheets, Discord, Linear, whatever you like — and map the event
   fields (`rating`, `body`, `author`, `app_version`, …) into it.
